---
title: "CARLA Autonomous Driving Challenge Leaderboard2.0を動かしてみた"
emoji: "🤖"
...
本記事では、CARLA Autonomous Driving Challenge Leaderboard2.0の概要を説明した上で、2024年入賞チームのソースコードが公開されたので試しに動かしてみた際のトライ&エラーの内容を紹介します。
なお、本記事は、主に以下の論文を参考に記載しています。
[Tackling CARLA Leaderboard 2.0 with End-to-End Imitation Learning](https://kashyap7x.github.io/assets/pdf/students/Zimmerlin2024.pdf)
...
グラボ: NVIDIA GeForce RTX 3090 (VRAM 24GB)
SSD: WD_Black 1TB
HDD: 2TB
...
![ranking2024](/images/ranking2024.png)
...
CARLA Leaderboard 2.0では、エージェントの性能を評価するために、
以下の3つの主要な指標が使用されます：
 ...
 **MinSp**: 周囲の交通の速度に基づいて計算される最低速度を維持することが求められます。この速度を維持できない場合、1から0.7の範囲でペナルティが発生します。
        この違反タイプは、簡略化のため上記の式には含まれていません。また、短いルートでの違反が避けられない場合が多いため、Town13 ShortベンチマークでのIS計算から除外されています
...
2024年大会でMAPトラック部門１位、SENSORSトラック部門２位のチームがコードと学習済みモデルを[公式github](https://github.com/autonomousvision/carla_garage?tab=readme-ov-file)にて公開されています。
以下では、上記リポジトリをクローンして、動確用に設定変更した下記のリポジトリに基づいて、動確した際のトライ&エラーの内容を共有します。
動確用コード：[carla_garage](https://github.com/akodama428/carla_garage.git)
ブランチ：leaderboard_2

このコードで用いられているアーキテクチャは以下の通りで、LidarとCameraデータをTransfuserでエンコードしたデータをTransformerDecoderに入力し、
MLP/GRUを介して他車両や人を回避した目標軌道を生成しています。
最終的な車両への制御指示値は、下図には記載されていませんが、上記で生成した目標軌道をPIDコントローラに入力し、車両操作量を演算する構成となっていました。
![trans_fuser](/images/trans_fuser_pp.png)
...
出力画像をつなぎ合わせて動画にすると、以下のようになり、路駐している車が急にドアを開くようなシーンであってもうまく回避できています。
下記の動画では、zennの容量制限のため最初の数秒のみを切り取っていますが、
実際には、右左折はもちろん、工事現場や自転車が走行する道路など、様々なシーンを走行するので、ぜひ手元で動かしてみてください。
![実行時の動画](/images/default.gif)

## 追加学習
苦手なシーンを追加するためにデータを生成します。
有効な学習データを収集するため、シーン別に細かくシナリオを切って実行可能なBench2Driveと、地図データや他車両の位置情報等を使用したデータ生成用のAgentとしてPDM Liteを使用します。基本はReadme記載の通りで動くはずですが、パス設定等でハマったので、私の個人PC環境で動かした際の設定ファイルを共有しつつ、手順を簡単にまとめます。
...
PDM Liteは、以下のstart_expert_local.shを実行することで生成します。
（こちらは、上記とは別のリポになるので、手元で動かす際は別途クローンし、readmeに従って仮想環境を構築してください）
...
```
![tensor_board](/images/tensor_board.png)

以上で追加学習自体はできます。今回はとりあえず動かすことを目的にお試しで実施したので、データ量が足りず再学習後の性能は悪化しています。
今後はどのようなデータセットで追加学習していくべきかを検討していきたいと思います。

## まとめ

個人的な所感ですが、自宅PCでデータ駆動型の自動運転システムが実行できることに感動しつつ、合流や右左折のシーンでは他車両と衝突するシーンも見られたため、改善の余地はまだまだありそうだなという印象です。CARLAの環境を使い、手元で動かしながら公開コードを確認することで、楽しみながら最新技術動向をキャッチアップできると思うので、ぜひ試してみてください！